(function () {
	// Increments a number in the URL by 1
	...
	if (found) {
		...
		const numPlusOne = String(Number(num) + 1);
		if (numPlusOne.length > num.length) {
			// Remove one 0 from the padding
			zeroes = zeroes.substring(0, zeroes.length - 1);
		}

		location = before + zeroes + numPlusOne + after;
	} else {
